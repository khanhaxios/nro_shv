...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(MySession session, String name, byte gender, int hair) {
        try {
            ...
            Service.gI().sendThongBaoOK(session, "Lỗi khi tạo hãy thử lại sau");
    ...
    public static void saveisBienHinh(Player player) {
        try (Connection con = GirlkunDB.getConnection(); PreparedStatement ps = con.prepareStatement("update player set isbroly = ? where id = ?");) {
            ps.setInt(1, player.isbroly);
