...
//                if (it.isNotNullItem() && it.template.id == item.template.id) {
    ...
    public void setxd(Player player) {
     ...
     //______________________________ đồ sét  thần linh ___________________________
    public void dotltd(Player player) {  // đồ thần linh
        NpcService.gI().createMenuConMeo(player, ConstNpc.menutltd, -1,
                                "Bạn có Muốn Nhận Không", "Nhận ngay", "Từ chối");
    }
    
    public void dotlnm(Player player) { // đồ tl 
        NpcService.gI().createMenuConMeo(player, ConstNpc.menutlnm, -1,
                                "Bạn có Muốn Nhận Không", "Nhận ngay", "Từ chối");
    }
    
    public void dotlxd(Player player) { // đồ thần
        NpcService.gI().createMenuConMeo(player, ConstNpc.menutlxd, -1,
                                "Bạn có Muốn Nhận Không", "Nhận ngay", "Từ chối");
    }
