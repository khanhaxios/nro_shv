...
public class Manager {
    ...
    public static final byte[] idsMapCold = {};
    ...
    private void initMap() {
        ...
        new Thread(() -> { //barcoll giảm thread scr
            try {
...
//                RUBY_REWARDS.add(Util.sendDo(861, 0, new ArrayList<>()));
            ...
            while (rs.next()) {
                ...
                switch (query) {
                    ...
                    case queryTopNap:
                        ...
                        top.setInfo1(Util.numberToMoney(rs.getLong("tongnap")) + " VND");
                        break;
                    case queryTopCS:
    ...
    public void loadProperties() throws IOException {
        ...
        Object value = null;
        //###Config sv
        if ((value = properties.get("server99.barcoll.port")) != null) {
            ServerManager.PORT = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.barcoll.name")) != null) {
            ServerManager.NAME = String.valueOf(value);
        }
        if ((value = properties.get("server99.barcoll.sv")) != null) {
            SERVER = Byte.parseByte(String.valueOf(value));
        }
        String linkServer = "";
        for (int i = 1; i <= 10; i++) {
            value = properties.get("server99.barcoll.sv" + i);
            if (value != null) {
                linkServer += String.valueOf(value) + ":0,";
            }
        }
        DataGame.LINK_IP_PORT = linkServer.substring(0, linkServer.length() - 1);
        if ((value = properties.get("server99.barcoll.waitlogin")) != null) {
            SECOND_WAIT_LOGIN = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server99.barcoll.maxperip")) != null) {
            MAX_PER_IP = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.barcoll.maxplayer")) != null) {
            MAX_PLAYER = Integer.parseInt(String.valueOf(value));
        }
        if ((value = properties.get("server99.barcoll.expserver")) != null) {
            RATE_EXP_SERVER = Byte.parseByte(String.valueOf(value));
        }
        if ((value = properties.get("server99.barcoll.local")) != null) {
